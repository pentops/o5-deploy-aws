// Code generated by protoc-gen-go-messaging. DO NOT EDIT.

package awsinfra_tpb

// Service: ECSRequestTopic
// Method: RunECSTask

func (msg *RunECSTaskMessage) MessagingTopic() string {
	return "o5-aws-command_request"
}
func (msg *RunECSTaskMessage) MessagingHeaders() map[string]string {
	headers := map[string]string{
		"grpc-service": "/o5.awsinfra.v1.topic.ECSRequestTopic/RunECSTask",
		"grpc-message": "o5.awsinfra.v1.topic.RunECSTaskMessage",
	}
	if msg.Request != nil {
		headers["o5-request-reply-to"] = msg.Request.ReplyTo
	}
	return headers
}

// Service: ECSReplyTopic
// Method: ECSTaskStatus

func (msg *ECSTaskStatusMessage) MessagingTopic() string {
	return "o5-aws-command_reply"
}
func (msg *ECSTaskStatusMessage) MessagingHeaders() map[string]string {
	headers := map[string]string{
		"grpc-service": "/o5.awsinfra.v1.topic.ECSReplyTopic/ECSTaskStatus",
		"grpc-message": "o5.awsinfra.v1.topic.ECSTaskStatusMessage",
	}
	if msg.Request != nil {
		headers["o5-reply-reply-to"] = msg.Request.ReplyTo
	}
	return headers
}
